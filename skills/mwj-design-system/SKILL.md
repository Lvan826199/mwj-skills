...
**规范文档位置：** `MWJ_Design_System.md`（18 章完整规范）
...
**Step 1 — 读取 MWJ 设计规范**
读取 `MWJ_Design_System.md`，确认当前任务适用的 Token、组件、布局规范。
...
```bash
# 图表类型推荐
python .claude/skills/ui-ux-pro-max-new/scripts/search.py "trend comparison timeline funnel" --domain chart

# 动效/加载/UX 规范
python .claude/skills/ui-ux-pro-max-new/scripts/search.py "animation loading accessibility z-index" --domain ux

# Vue 栈最佳实践
python .claude/skills/ui-ux-pro-max-new/scripts/search.py "dashboard admin saas" --stack vue

# Web 可访问性规范
python .claude/skills/ui-ux-pro-max-new/scripts/search.py "aria focus keyboard semantic" --domain web
```
